...
public class faculty_dao {
	...
	public boolean checkFaculty(String username,String password) {
		try (MongoClient mongoClient = new MongoClient(new MongoClientURI(database_url))) {
            ...
            MongoCollection<Document> usersCollection = database.getCollection("facultylogin"); // Replace with your collection name

            Document user = usersCollection.find(Filters.eq("email", username)).first();
	...
	public boolean delete_faculty(String email) {
		try (MongoClient mongoClient = new MongoClient(new MongoClientURI(database_url))) {
